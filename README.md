...
本專案使用 Z.AI (智譜 AI) 的 GLM-ASR 模型進行語音轉文字：

- **優點**: 內建上下文理解，轉錄結果直接可用，無需額外潤飾
- **限制**: 單次請求最長 30 秒
- **解決方案**: 實作自動音訊分割，長音訊會自動分段處理後合併
- **官方 SDK**: 使用 `zhipuai` Python SDK 呼叫 API

### 音訊處理流程

```
錄音 → WAV 格式 (16kHz) → 分割成 30 秒片段 → 轉錄 → 合併結果 → 複製到剪貼簿
```
